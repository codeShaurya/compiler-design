...
vector<string> buildFirst(string nt, string s)
{
  ...
  if (nt == s)
    return ans;
  // INFO Avoid circular dependency
  if (s == "")
  {
    ans.push_back("");
    return ans;
  ...
  string fl(1, s[0]);
  // INFO Take first character and check if it is non terminal.
  if (mp.find(fl) == mp.end())
  {
    ans.push_back(fl);
    return ans;
  }
  // INFO If first of first non terminal in RHS doesnt exist then build it
  if (First.find(fl) == First.end())
  {
    buildFirstNT(fl);
  }
  // INFO RHS's first set ki saari string ko answer mein daal do
  for (string s : First[fl])
    ans.push_back(s);
  auto it = find(ans.begin(), ans.end(), "");
  // INFO  agar epsillon hai toh uske next waale ka bhi check karo.
  if (it != ans.end())
  {
    ans.erase(it);
...
void buildFirstNT(string nt)
{
  ...
  for (string s : mp[nt])
  {
    //INFO Select every production of nt, and get First of the RHS.
...
void buildFollow(string nt)
{
  ...
  for (auto it : mp)
  {
    ...
    {
      int found;
      // INFO Jab tk nt production mein exist karta hai, tb tk follow dhoondo is production mein nt ka
